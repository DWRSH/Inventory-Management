...
const Payment = require('../models/payment.model'); // 1. (नया) Payment मॉडल इम्पोर्ट करें
...
const createSale = async (req, res) => {
  ...
  if (paymentStatus === 'Paid' && amountDue !== 0) { return res.status(400).json({ message: 'For "Paid" status, amount paid must equal total amount.' }); }
  if (paymentStatus === 'Unpaid' && paidAmount !== 0) { return res.status(400).json({ message: 'For "Unpaid" status, amount paid must be 0.' }); }
  if (paymentStatus === 'Partial' && (paidAmount <= 0 || paidAmount >= total)) { return res.status(400).json({ message: 'For "Partial" status, amount paid must be greater than 0 and less than total.' }); }
  ...
  try {
    ...
    // 5. (नया) अगर पेमेंट हुई है (0 से ज़्यादा), तो Payment रिकॉर्ड बनाएँ
    let newPaymentId = null;
    if (paidAmount > 0) {
      const newPayment = new Payment({
        customer: customerId,
        amountPaid: paidAmount,
        paymentType: 'Sale', // यह पेमेंट सेल (बिल) के साथ आई है
        sale: createdSale._id // इस बिल से जुड़ी है
      });
      const createdPayment = await newPayment.save({ session });
      newPaymentId = createdPayment._id;
    }

    // 6. Customer की history को अपडेट करें
    const customerUpdateOperations = {
      $push: { purchaseHistory: createdSale._id }
    };
    if (newPaymentId) {
      // अगर पेमेंट बनी है, तो उसे भी हिस्ट्री में जोड़ें
...
// ... (getSalesHistory function जैसा था वैसा ही है) ...
